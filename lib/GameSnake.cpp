...
RenderMap GameSnake::getRenderMap()
{
  ...
  for (int y = 0; y < size.y; y++)
  {
    for (int x = 0; x < size.x; x++)
    {
      ...
      if (value == 0)
        now = RenderObject::CLEAR;
      else if (value == int(RenderObject::BECON))
        now = RenderObject::BECON;
      else if (value == int(RenderObject::APPLE))
        now = RenderObject::APPLE;
      else if (value == 1)
        now = RenderObject::TAIL;
      else if (value == score)
        now = RenderObject::HEAD;
...
bool GameSnake::tick()
{
  ...
  {
    score++;
    map.setValue(newPosition, score);
    return true;
  }
